...
Reads a CycloneDX or SPDX JSON document and reports, per document:
  ...
  * the augmentation surface -- the document-level metadata augmentation is
    responsible for. The first survey scored five root-component fields and
    nothing else, so an augmentation regression could not have been seen.
    Note that with no token the sbomify API provider skips itself, so
    metadata.supplier and metadata.manufacturer are expected to stay empty;
    that is a gap in what the harness exercises, not a defect, and it is
...
def inspect_cyclonedx(doc):
    ...
    # Augmentation writes here rather than into components. Recorded as
    # presence, not counts: one supplier block either arrived or it did not.
    aug = {f"metadata.{k}": bool(meta.get(k)) for k in
           ("supplier", "manufacturer", "authors", "licenses", "lifecycles", "tools")}
    aug["root.licenses"] = bool(root.get("licenses"))
    aug["root.supplier"] = bool(root.get("supplier"))
    aug["root.externalReferences"] = bool(root.get("externalReferences"))
    aug["root.vcs"] = any((r or {}).get("type") == "vcs"
                          for r in (root.get("externalReferences") or []))
