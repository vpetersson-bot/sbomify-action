...
res=$EVAL_ROOT/meta/$key.json
log=$EVAL_ROOT/logs/$key.log
repo=$EVAL_ROOT/repos/$key
...
lockdir=$EVAL_ROOT/locks/$key
mkdir -p "$EVAL_ROOT/locks"
if ! mkdir "$lockdir" 2>/dev/null; then
  echo "BUSY $slug (claimed by another worker)"; exit 0
fi

mkdir -p "$EVAL_ROOT/meta" "$EVAL_ROOT/logs" "$EVAL_ROOT/out"
: > "$log"
...
release_lock() { rmdir "$lockdir" 2>/dev/null; return 0; }
...
if [ "$ecosystem" = "docker" ]; then
  out=$EVAL_ROOT/out/$key.cdx.json
  timeout $RUN_TIMEOUT docker run --rm \
    -v "$CACHE":/cache -v "$EVAL_ROOT/out":/out \
...
run_action() {  # $1 = extra env flag value ("" or "1")
  local fb=$1
  timeout $RUN_TIMEOUT docker run --rm \
    -v "$repo":/workspace -v "$CACHE":/cache -v "$EVAL_ROOT/out":/out \
    -e HOME=/cache/home -e XDG_CACHE_HOME=/cache/xdg -e SBOMIFY_CACHE_DIR=/cache/enrichment \
