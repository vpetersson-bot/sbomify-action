...
mkdir -p "$EVAL_ROOT"/{meta_jvm,logs_jvm,out_jvm,slots}
...
for _try in $(seq 1 240); do
  for i in $(seq 0 $((SLOTS-1))); do
    if mkdir "$EVAL_ROOT/slots/$i.lock" 2>/dev/null; then slot=$i; break 2; fi
  done
  sleep 15
done
[ -z "$slot" ] && { echo "NOSLOT $slug (waited 60m)"; exit 0; }
cache=$EVAL_ROOT/slots/$slot.cache
mkdir -p "$cache"
...
cleanup() {
  ...
  rmdir "$EVAL_ROOT/slots/$slot.lock" 2>/dev/null
  return 0
}
trap cleanup EXIT
